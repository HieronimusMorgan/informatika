...
<div id="layoutSidenav_content">
  <main>
    <div class="container-fluid">
      ...
      <div class="card-body">
        <a class="btn btn-primary mb-2" data-toggle="modal" data-target="#ModalInput">Tambah Ujian</a>
                ...
                <tr>
                  ...
                  <td>
                   ...
                   <a href="<?php echo base_url(); ?>rekomendasi/deletedetailjadwal/<?= $row['id']; ?>/<?= $row['idJadwal'] ?>"
                    class="badge badge-danger"onclick="return confirm('Are you sure you want to delete ?');">Delete</a>
                  </td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
  ...
  <script>
    ...
    function matakuliah(id, semester,idJadwal){
      ...
      $.ajax({
        ...
        method: "POST",
        data : {id : id, semester : semester,idJadwal : idJadwal},
        async : false,
        dataType : 'json',
        success : function(data){
          ...
          for (i = 0; i<data.length; i++){  
            ...
            html += '<option value="'+data[i].idMakul+'">'+data[i].nama+' / '+data[i].kelas+'</option>';                
          }
          $('#matkul').html(html);
        }
      });
    ...
    $('#matkul').change(function(){
      ...
      $.ajax({
        ...
        success : function(data){
          ...
          var i;
          for (i = 0; i<data.length; i++){        
            html += '<option value='+data[i].idRuangan+'>'+data[i].ruangan+'</option>';               
          }
          $('.kelas').html(html);
        }
      });
